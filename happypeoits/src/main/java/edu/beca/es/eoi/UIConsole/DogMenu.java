...
public class DogMenu {
	...
	public void printDogMenu(String name, String location, String description, int typePet, String photo, int idPet) {
		int age = 0;
		int years = 0;
		int months = 0;
		log.info("Entra en el menu de perro para acabar de completar los datos de la mascota");
		System.out.println("¿Cuantos años tiene el perro?");
		try {
			...
			printDogMenu(name, location, description, typePet, photo, idPet);
		}
		...
		try {
			...
			PETCONTROLLER.saveDog(name, location, description, typePet, photo, age, idPet);
	...
	public void readByIdResult(Pet pet) {
		int ageMonths = pet.getDog().getAge() * 12;
	...
	public boolean uploadDog(int idPet, String newname, String newlocation, String newdescription, String newOwnerName,
